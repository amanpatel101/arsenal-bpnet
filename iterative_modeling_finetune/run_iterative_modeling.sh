...
bpnet fit-arsenal -p "$CONFIG_JSON" -a "$MODEL_PATH" -l "loci.txt" -n "negatives.txt" --finetune
...
for SIZE_PERCENT in "${SIZES[@]}"; do
    ...
    cd "$ARSENAL_DIR"
    bpnet fit-arsenal -p "$CONFIG_JSON" -a "$MODEL_PATH" -l "loci_sampled.txt" -n "negatives_sampled.txt" --finetune
