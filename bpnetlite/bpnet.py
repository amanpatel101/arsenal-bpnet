...
class ArsenalBPNet(BPNet):
	'''
	A model which initializes BPNet inputs based on ARSENAL embeddings and either probes or fine-tunes. 
	'''
	def __init__(self, arsenal_model, seq_input_size=2114, arsenal_input_size=350, finetune=False, input_dim=512, input_type="embedding", n_filters=64, n_layers=8, n_outputs=2, 
		n_control_tracks=2, count_loss_weight=1, profile_output_bias=True,
		count_output_bias=True, name=None, trimming=None, verbose=True):
		super(ArsenalBPNet, self).__init__(n_filters, n_layers, n_outputs, n_control_tracks, count_loss_weight, profile_output_bias, count_output_bias, name, trimming, verbose)
		...
		self.iconv = torch.nn.Conv1d(input_dim, n_filters, kernel_size=21, padding=10)
	...
	def forward(self, X, X_ctrl=None):
		tokens = self.one_hot_to_tokens(X.transpose(1,2))
		if self.input_type == "embedding":
			X_embs = self.get_embeddings(tokens).transpose(1,2)
		elif self.input_type == "likelihood":
			X_embs = self.get_likelihoods(tokens).transpose(1,2)

		return super().forward(X_embs, X_ctrl)
